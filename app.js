...
/* Revised version to utilize refactored data management module and its data service */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentWeekStartDateEl = document.getElementById(
    "current-week-start-date"
  );
  const currentWeekSummaryContent = document.getElementById(
  ...
  const editTotalsItemTemplate = document.getElementById(
  ...
  // --- State Management ---
  function loadState() {
    const savedState = dataService.loadState();

    const today = dataService.getTodayDateString();

    const currentWeekStart = dataService.getWeekStartDate(new Date()); // *** USE RENAMED FUNCTION (relies on default 'Sunday') ***

    state.currentDayDate = savedState.currentDayDate || today;
    state.currentWeekStartDate =
      savedState.currentWeekStartDate || currentWeekStart;
    ...
    foodGroups.forEach((group) => {
      ...
      if (
        group.frequency === "week" ||
        group.frequency === "day" ||
        group.id === "butter_margarine" ||
        group.id === "wine"
      ) {
        // All groups need weekly tracking (daily ones summed up)
  ...
  async function checkDateAndResetCounters() {
    ...
    const currentWeekStartStr = dataService.getWeekStartDate(today); // *** USE RENAMED FUNCTION (relies on default 'Sunday') ***
    ...
    if (state.currentWeekStartDate !== currentWeekStartStr) {
      console.log(
        `Week reset triggered: Stored week ${state.currentWeekStartDate}, Current week ${currentWeekStartStr}`
      );
    ...
    console.log(
      `PRE-DAY-CHECK: Stored Date = '${
        state.currentDayDate
      }', Today's Date = '${todayStr}', Comparison Result = ${
    ...
    if (!weekResetOccurred && state.currentDayDate !== todayStr) {
      console.log(
        `Day reset triggered: Stored day ${state.currentDayDate}, Current day ${todayStr}`
      );
      ...
      foodGroups.forEach((group) => {
        if (
          ...
          // Only add counts for groups tracked daily
          if (state.dailyCounts[group.id] && state.dailyCounts[group.id] > 0) {
            state.weeklyCounts[group.id] =
              (state.weeklyCounts[group.id] || 0) + state.dailyCounts[group.id];
            console.log(
              `Adding ${state.dailyCounts[group.id]} from ${
                group.id
    ...
    if (stateChanged) {
      ...
      console.log(
        "State changed detected. Preparing to save state. Current date in state:",
        state.currentDayDate
      );
      saveState();
  ...
  async function archiveWeek(weekStartDate, weeklyTotals) {
    ...
    try {
      ...
      showToast(
        `Failed to archive week ${weekStartDate}: ${error.message}`,
        "error"
      );
  ...
  function renderUI() {
    try {
      ...
      console.log(
        "renderUI: Rendering dates from state. CurrentDay:",
        state.currentDayDate,
        "WeekStart:",
        state.currentWeekStartDate
      );
      ...
      if (dailyGoalsDateEl) {
        dailyGoalsDateEl.textContent = `${displayDate.toLocaleDateString(
          undefined,
          { weekday: "short" }
      ...
      if (weeklyGoalsDateEl) {
        ...
        weeklyGoalsDateEl.textContent = `Starts ${weekStartDateDisplay.toLocaleDateString(
          undefined,
          { weekday: "short" }
    ...
    currentWeekStartDateEl.textContent = `Starts ${weekStartDateDisplay.toLocaleDateString(
      undefined,
      { weekday: "short" }
    )}, ${weekStartDateDisplay.toLocaleDateString(undefined, {
      month: "short",
    ...
    foodGroups.forEach((group) => {
      ...
      if (group.type === "positive") {
        ...
        if (currentTotal > weeklyTarget * 0.75 && currentTotal <= weeklyTarget)
          statusClass = "limit-near"; // If > 75% of limit
  ...
  function renderHistory(weekIndex = -1) {
    ...
    console.log(
      "History data to render:",
      state.history.map((h) => h.weekStartDate)
    );
    console.log(
    ...
    const historyTargets =
      ...
      foodGroups.reduce((acc, group) => {
        ...
        console.log(
          `History Warning: Using current config fallback for targets in week ${weekData.weekStartDate}`
        );
        acc[group.id] = {
          target: group.target,
          frequency: group.frequency,
    ...
    const foodGroupsToDisplay = foodGroups.filter(
      (group) =>
        historyTargets[group.id] ||
  ...
  function setupEventListeners() {
    ...
    prevWeekBtn.addEventListener("click", () => {
      ...
      if (state.currentHistoryIndex < state.history.length - 1) {
        console.log(
          `Moving from history index ${state.currentHistoryIndex} to ${
            state.currentHistoryIndex + 1
          }`
        );
    ...
    if (editCurrentWeekBtn)
      editCurrentWeekBtn.addEventListener("click", () =>
  ...
  function handleCounterClick(event) {
    ...
    if (button.classList.contains("increment-btn")) {
      ...
      if (currentValue < oldValue) {
        // Only flag if value actually decreased
        valueChanged = true;
      }
    } else {
  ...
  function updateCount(groupId, newValue, isDaily, itemElement) {
    ...
    if (isDaily) {
      ...
      if (weeklyTotalValue)
        weeklyTotalValue.textContent = state.weeklyCounts[groupId] || 0;
    }

    saveState();
    renderCurrentWeekSummary(); // Update summary whenever counts change
    console.log(
      `Updated count for ${groupId}: Daily=${state.dailyCounts[groupId]}, Weekly=${state.weeklyCounts[groupId]}`
    );
  }
  ...
  function handleHistoryDatePick() {
    ...
    const targetWeekStart = dataService.getWeekStartDate(selectedDate); // *** USE RENAMED FUNCTION (relies on default 'Sunday') ***
    ...
    if (foundIndex !== -1) {
      ...
      if (state.currentHistoryIndex !== -1) {
        historyDatePicker.value =
          state.history[state.currentHistoryIndex].weekStartDate;
  ...
  async function loadHistoryData() {
    try {
      state.history = await dataService.getAllWeekHistory(); // Fetch and sort (newest first)
  ...
  function handleOutsideMenuClick(event) {
    ...
    if (
      ...
      mainMenu.classList.contains("menu-open")
    ) {
      closeMenu();
    }
  }
  ...
  function handleAboutClick() {
    ...
    // Add existing development information
    aboutContent += `
    ...
    // Fetch and display version inside the modal
    ...
    if (applyTestDateBtn) {
      applyTestDateBtn.addEventListener("click", () => {
        ...
        if (dateValue) {
          ...
          // Update application dates for the new test date
          updateDatesForTestMode();

          // Show a banner and refresh UI
          addTestModeBanner();
          renderUI();
          showToast(
            "Test date applied: " +
    ...
    if (resetTestDateBtn) {
      resetTestDateBtn.addEventListener("click", () => {
        ...
        // Update application dates for the real date
        ...
        state.currentDayDate = realTodayStr;
        state.currentWeekStartDate = dataService.getWeekStartDate(realToday);
        saveState();

        // Remove banner and refresh UI
        removeTestModeBanner();
        renderUI();
        showToast("Test mode disabled. Using real system date.", "success");
      });
    }
  }

  /**
   * Update application state to reflect the test date
   */
  async function updateDatesForTestMode() {
    // Only proceed if test mode is enabled
    if (!dataService.isTestModeEnabled()) return;

    // Get the new current day string
    const todayStr = dataService.getTodayDateString();

    // Get new week start date
    const currentWeekStartStr = dataService.getWeekStartDate(
      dataService.getCurrentDate()
    );

    console.log(`Updating application dates for test mode:`, {
      oldDayDate: state.currentDayDate,
      newDayDate: todayStr,
      oldWeekStart: state.currentWeekStartDate,
      newWeekStart: currentWeekStartStr,
    });
    ...
    // Update the state dates
    state.currentDayDate = todayStr;
    state.currentWeekStartDate = currentWeekStartStr;

    // If dates changed, we might need to trigger date-related logic
    if (datesChanged) {
      console.log("Test date caused date change - checking for resets");
      saveState();
      // Optional: Consider if you need to run checkDateAndResetCounters() here
      // Be careful as this may reset your counters if the date changed significantly
  ...
  function updateDevInfo() {
    ...
    document.getElementById(
      "dev-viewport-size"
    ).textContent = `${viewportWidth}px × ${viewportHeight}px`;
    ...
    document.getElementById(
      "dev-screen-size"
    ).textContent = `${screenWidth}px × ${screenHeight}px`;
  ...
  /**
   * Handle export operation using the enhanced dataService
   * This function exports all app data to a JSON file for backup or transfer
   */
  async function handleExport() {
    ...
    try {
      ...
      // Use data service to get all exportable data
      ...
      link.href = url;
      const timestamp = dataService.getTodayDateString(); // Use local date
  ...
  function addTestModeBanner() {
    ...
    banner.textContent = `TEST MODE: Using date ${dataService
      .getCurrentDate()
      .toLocaleDateString()}`;
    banner.style.cssText =
      "position: fixed; bottom: 0; left: 0; right: 0; background-color: #ffcc00; color: #000; text-align: center; padding: 5px; font-weight: bold; z-index: 1000;";
  ...
  function getDateRelationship(importDate, todayDate) {
    ...
    const importDateObj =
      importDate instanceof Date
        ? importDate
        : new Date(importDate + "T00:00:00");
    const todayDateObj =
      todayDate instanceof Date ? todayDate : new Date(todayDate + "T00:00:00");
    ...
    if (
      importDateObj.toISOString().split("T")[0] ===
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "Invalid file structure. Required: 'currentState' object and 'history' array."
          );
        }
        ...
        // Get existing state for potential preservation
        const existingState = dataService.loadState();
        ...
        if (
          !validateWeekStartDate(importedData.currentState.currentWeekStartDate)
        ) {
          console.warn(
            `Imported currentWeekStartDate is not a Sunday: ${importedData.currentState.currentWeekStartDate}`
          );
        }
        ...
        switch (dateRelationship) {
          ...
          case "SAME_WEEK":
            actionDescription =
              "UPDATE current week totals and REPLACE history";
            break;
          case "PAST_WEEK":
            actionDescription =
        ...
        switch (dateRelationship) {
          case "SAME_DAY":
            // Clear all data and use imported state as-is
            await dataService.clearHistoryStore();
            currentStateToUse = JSON.parse(
              JSON.stringify(importedData.currentState)
            );

            // Import history
            for (const weekData of importedData.history) {
              try {
                if (isValidHistoryRecord(weekData)) {
                  await dataService.saveWeekHistory(weekData);
                  importCount++;
                }
              } catch (error) {
                console.error(`Error saving history week:`, error);
              }
            }
            break;

          case "SAME_WEEK":
            // Clear history but update current week data
            await dataService.clearHistoryStore();
          ...
          case "PAST_WEEK":
            ...
            // Check if imported current state week should be archived
            if (
              importedData.currentState.weeklyCounts &&
              Object.values(importedData.currentState.weeklyCounts).some(
                (val) => val > 0
              )
            ) {
              const weekToArchive = {
                ...
                weekStartDaySetting:
                  importedData.currentState.weekSetting || "Sunday",
              ...
              try {
                await dataService.saveWeekHistory(weekToArchive);
                importCount++;
              } catch (error) {
            ...
            for (const weekData of importedData.history) {
              try {
                if (isValidHistoryRecord(weekData)) {
                  ...
                  const existingWeek = state.history.find(
                    (w) => w.weekStartDate === weekData.weekStartDate
                  );
                  if (!existingWeek) {
                    await dataService.saveWeekHistory(weekData);
                    importCount++;
        ...
        if (Object.keys(currentStateToUse).length > 0) {
          dataService.saveState(currentStateToUse);
        }
  ...
  // Helper function to validate history records
  function isValidHistoryRecord(weekData) {
    return (
      weekData &&
  ...
  function renderEditTotalsList() {
    ...
    foodGroups.forEach((group) => {
      const item = editTotalsItemTemplate.content
        .cloneNode(true)
  ...
  function handleEditTotalsItemClick(event) {
    ...
    if (!groupId) {
      console.error(
        "Edit button clicked, but no groupId found in dataset.",
        button
      );
      return;
    }
    ...
    if (itemElement) {
      ...
      if (totalSpan) {
        ...
        console.error(
          "Could not find totalSpan element within item:",
          itemElement
        );
      }
    } else {
