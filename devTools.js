...
class DevTools {
  ...
  /**
   * Get information about all JavaScript modules in the application
   * @returns {Promise<Array>} Array of module information objects
   */
  async getModuleInformation() {
    // List of core application modules to check
    const moduleList = [
      "app.js",
      "dataService.js",
      "stateManager.js",
    ...
    for (const modulePath of moduleList) {
      try {
        ...
        // First try a range request to get file info efficiently
        try {
          ...
          if (response.ok || response.status === 206) {
            ...
            // Try to get file size from Content-Range header (e.g., "bytes 0-0/1234")
            if (contentRange) {
              const rangeMatch = contentRange.match(/bytes \d+-\d+\/(\d+)/);
              if (rangeMatch) {
                const bytes = parseInt(rangeMatch[1]);
                fileSize = this.formatFileSize(bytes);
              }
            }
            // Fallback to Content-Length for servers that don't support range requests
            else if (contentLength) {
              const bytes = parseInt(contentLength);
              fileSize = this.formatFileSize(bytes);
            }
          } else if (response.status === 416) {
            // Range not satisfiable - try a regular HEAD request
          ...
          // Range requests not supported, fall back to HEAD request
          try {
            response = await fetch(modulePath, {
              method: "HEAD",
              cache: "no-cache",
            ...
            // If HEAD also fails, mark as error
            status = "Network Error";
            logger.warn(
              `Both range and HEAD requests failed for ${modulePath}:`,
              headError
            );
          }
        }
        ...
        logger.warn(`Error checking module ${modulePath}:`, error);
        moduleInfo.push({
          name: modulePath,
          lastModified: "Error",
          fileSize: "Unknown",
    ...
    return moduleInfo.sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  /**
   * Generate HTML for module information display
   * @param {Array} moduleInfo - Array of module information objects
   * @returns {string} HTML string for module information
   */
  generateModuleInfoHtml(moduleInfo) {
    let html = `
      <div style="margin-top: 10px;">
        <h5 style="margin: 5px 0;">Application Modules (${moduleInfo.length})</h5>
        <div style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 8px; font-size: 11px;">
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background-color: #f5f5f5;">
                <th style="text-align: left; padding: 4px; border-bottom: 1px solid #ddd;">Module</th>
                <th style="text-align: left; padding: 4px; border-bottom: 1px solid #ddd;">Size</th>
                <th style="text-align: left; padding: 4px; border-bottom: 1px solid #ddd;">Last Modified</th>
                <th style="text-align: left; padding: 4px; border-bottom: 1px solid #ddd;">Status</th>
              </tr>
            </thead>
            <tbody>
    `;
    ...
    moduleInfo.forEach((module) => {
      ...
      html += `
        <tr>
          <td style="padding: 3px; border-bottom: 1px solid #eee; font-family: monospace;">
            ${module.name}
          </td>
          <td style="padding: 3px; border-bottom: 1px solid #eee;">
            ${module.fileSize}
          </td>
          <td style="padding: 3px; border-bottom: 1px solid #eee; font-size: 10px;">
            ${module.lastModified}
          </td>
          <td style="padding: 3px; border-bottom: 1px solid #eee; color: ${statusColor};">
            ${module.status}
          </td>
        </tr>
      `;
